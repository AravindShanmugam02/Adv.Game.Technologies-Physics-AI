...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	Vector3 contactVelocity = fullVelocityB - fullVelocityA; //Also referred as relative velocity -- VR=VB−VA

	//Impulse Method (with restitution containing elasticity) (We get this elasticity as a unique value for separate objects)
	...
	//Coefficient of restitution is where we apply elasticity
	float cRestitution = physA->GetElasticity() * physA->GetElasticity(); //0.66f; // disperse some kinectic energy
	// Equation1: j =− (1+e)((VB−VA)⋅n) / (1/massA + 1/massB) we additionally include inertia effect for the two objects
	float j = (-(1.0f + cRestitution) * impulseForce) / (totalMass + angularEffect); //-(1.0f + cRestitution) *  // First Coefficient Of Restitution


	//Calculating the amount of Frictional Impulse
	//Get the tangent normal vector -- t=VR−(VR⋅n)∗n
	Vector3 tangentNormal = contactVelocity - (p.normal * Vector3::Dot(contactVelocity, p.normal));
	tangentNormal = tangentNormal.Normalised(); // Normalised because --> The tangent normal, this is a unit vector that is perpendicular to the contact normal.

	//Magnitude to be applied along the Friction vector -- Similar to impulseForce but with tangent normal
	float impulseWithTanNorm = Vector3::Dot(contactVelocity, tangentNormal);

	// now to work out the effect of inertia .... With TangentNormal
	Vector3 inertiaAWithTanNorm = Vector3::Cross(physA->GetInertiaTensor() * Vector3::Cross(relativeA, tangentNormal), relativeA);
	Vector3 inertiaBWithTanNorm = Vector3::Cross(physB->GetInertiaTensor() * Vector3::Cross(relativeB, tangentNormal), relativeB);
	float angularEffectWithTanNorm = Vector3::Dot(inertiaAWithTanNorm + inertiaBWithTanNorm, tangentNormal);

	// PythagoreanSolve = A^2 + B^2 = C^2, solving for C given A and B
	// Getting the PythagoreanSolve and then finding SQRT to get the averageFriction value.
	// (Basically we can't use C^2, so we do SQRT to get C)
	float avgFriction = Maths::Clamp(std::sqrt(physA->GetFriction() * physA->GetFriction() + physB->GetFriction() * physA->GetFriction()), 0.1f, 1.0f) ;

	// Equation2: j =− (1+e)((VB−VA)⋅t) / (1/massA + 1/massB) we additionally include inertia effect for the two objects and this is done with tangent normal.
	float jt = (-(1.0f + cRestitution) * impulseWithTanNorm) / (totalMass + angularEffectWithTanNorm); //-(1.0f + cRestitution) * // Second Coefficient Of Restitution

	// Clamp magnitude of friction and create impulse vector
	//Vector3 fullImpulse;
	//if (abs(jt) < j * avgFriction)
	//{
		//fullImpulse = tangentNormal * jt;
	//}
	//else
	//{
		//fullImpulse = p.normal * j * avgFriction;
	//}

	Vector3 fullImpulse = (p.normal * j) + (tangentNormal * jt) * avgFriction; // *(1.0f + cRestitution);
...
/*void PhysicsSystem::PenaltyResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const
{
	...
	float springResting = 0.0f;
	float x = p.penetration - springResting; // Spring Stretch length is difference between the penetration distance and resting length.
	float K = -1.0f; // Spring constant
	float f = x*K; // Spring Force is proportional to penetration distance.
	float inverseMassA = physA->GetInverseMass();
	float inverseMassB = physB->GetInverseMass();

	//physA->AddForce()

	float penaltyAccelerationA = f / inverseMassA;
	float penaltyAccelerationB = f / inverseMassB;

	Vector3 force = Vector3::Dot(f, p.normal);;

	Vector3 relativeA = p.localA;
	Vector3 relativeB = p.localB;

	Vector3 penaltyTorqueA = Vector3::Cross(localPos, addedForce);
	...
	tree.OperateOnContents([&](std::list<QuadTreeEntry<GameObject*>>& data)
		{
			...
			//try
			std::cout << "Number of broadPhaseCollisions :: " << broadphaseCollisions.size() << " :: ";
		});
}
...
void PhysicsSystem::NarrowPhase() {
	...
	//try
	std::cout << "Number of NarrowPhaseCollisions :: " << allCollisions.size() << " :: ";
}
